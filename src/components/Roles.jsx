...
import CEO from "../assets/roles/ceo.png";
import CFO from "../assets/roles/cfo.png";
import CMO from "../assets/roles/cmo.png";
...
const Roles = () => {
    ...
    return (
        <div
            id="roles"
            className="min-h-2/3 flex items-center justify-center bg-yellow-50 p-6 sm:p-10"
        >
                    ...
                    {roles.map((role, index) => {
                        ...
                        return (
                            <motion.div
                                ...
                                viewport={{ once: true }}
                                className={`relative group ${bg} p-6 rounded-xl border-3 border-amber-950 transition-transform duration-300 hover:scale-105`}
                                style={{
                                    rotate: rotation,
                            ...
                            >
                                <img
                                    src={tape}
                                    alt="tape"
                                    className={`${tapeStyle} min-w-25`}
                                />
                                <img
                                    src={role.image}
                                    alt={role.title}
                                    className="mx-auto h-30 lg:h-40 rounded-lg mb-4"
                                />
                                <h3 className="text-xl lg:text-2xl font-bold text-[var(--color-black)] mb-3">
                                    {role.title}
                                </h3>
                                <p className="font-semibold text-[var(--color-maroon)] text-sm lg:text-base leading-relaxed">
                                    {role.description}
                                </p>
                            </motion.div>
                        );
